<script>
    ...
    const moved = (point, withCursorForce = true) => {
        const x =
            point.x + point.wave.x + (withCursorForce ? point.cursor.x : 0)
        const y =
            point.y + point.wave.y + (withCursorForce ? point.cursor.y : 0)
        return {
        ...
        lines.forEach(points => {
            points.forEach(p => {
                ...
                if (dist < limit) {
                    ...
                    p.cursor.vx +=
                        Math.cos(mouse.a) * f * limit * mouse.vs * 0.00065
                    p.cursor.vy +=
                        Math.sin(mouse.a) * f * limit * mouse.vs * 0.00065
                }
